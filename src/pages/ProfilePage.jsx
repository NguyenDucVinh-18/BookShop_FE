          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  <Text className="order-total-label">Tổng thanh toán</Text>
...
function MapResizer({ trigger }) {
  ...
  useEffect(() => {
    // Gọi invalidateSize mỗi khi trigger thay đổi (modal mở, GPS lấy xong...)
    setTimeout(() => {
      map.invalidateSize();
    }, 200);  // 200ms đủ để modal mở xong animate
  }, [map, trigger]);

  return null;
}

/* ================= FIX LEAFLET ICON (BẮT BUỘC) ================= */
...
/* ================= MAP PICKER ================= */
function MapPicker({ center, onPick, zoom = 18 , trigger = false}) {
...
async function reverseGeocode(lat, lng) {
  const res = await fetch(
    ...
    { headers: { "User-Agent": "address-picker" } }
  );
  return res.json();
}
...
const AddressesTab = () => {
  ...
  const [mapCenter, setMapCenter] = useState({
    lat: 10.8231, // Trung tâm TP.HCM (fallback)
    lng: 106.6297,
  });
  ...
  const handleAddAddress = () => {
    ...
    // Tự động lấy GPS ngay khi mở modal thêm mới
  ...
  const handlePickLocation = async ({ lat, lng }) => {
    ...
    form.setFieldsValue({
      ...
      district: addr.city_district || addr.neighbourhood || "",
      city: addr.city || addr.state || "",
    });

    message.info("Đã lấy địa chỉ từ bản đồ, vui lòng kiểm tra lại");
  };


  const getCurrentLocationAndSetup = () => {
    if (!navigator.geolocation) {
      message.warning(
        "Trình duyệt không hỗ trợ định vị. Bạn có thể click chọn trên bản đồ."
      );
      return;
    }

    message.loading("Đang xác định vị trí của bạn...", 0);
    ...
    const watchId = navigator.geolocation.watchPosition(
      async (pos) => {
        ...
        // Cập nhật center và zoom sát hơn
        setMapCenter({ lat: latitude, lng: longitude });
        setMapZoom(18); // Zoom sát khi có GPS chính xác

        message.success(
          `Đã xác định vị trí (độ chính xác ~${accuracy.toFixed(0)}m)`
        );

        // TỰ ĐỘNG reverse geocode và điền form luôn (siêu tiện!)
        try {
          const data = await reverseGeocode(latitude, longitude);
          const addr = data.address || {};

          form.setFieldsValue({
            street: addr.road || addr.road_name || "",
            ward: addr.suburb || addr.village || addr.neighbourhood || "",
            district: addr.city_district || addr.county || addr.district || "",
            city: addr.city || addr.state || "TP. Hồ Chí Minh",
          });

          message.info(
            "Đã tự động điền địa chỉ gần đúng. Bạn có thể chỉnh sửa hoặc click lại trên bản đồ để chính xác hơn."
          );
        } catch (err) {
          message.info(
            "Đã xác định vị trí. Hãy click đúng nhà bạn trên bản đồ để lấy địa chỉ."
          );
        }
      },
      (error) => {
        ...
        switch (error.code) {
          case error.PERMISSION_DENIED:
            message.warning(
              "Bạn đã từ chối chia sẻ vị trí. Bản đồ sẽ hiển thị toàn TP.HCM."
            );
            break;
          case error.POSITION_UNAVAILABLE:
          case error.TIMEOUT:
            message.warning(
              "Không thể lấy vị trí GPS. Bạn vẫn có thể click chọn trên bản đồ."
            );
            break;
          default:
            message.warning(
              "Không lấy được vị trí. Hãy click chọn khu vực của bạn trên bản đồ."
            );
        }

        // Fallback: giữ center giữa TP.HCM, zoom xa
        setMapCenter({ lat: 10.8231, lng: 106.6297 });
        setMapZoom(12);
      },
      {
  ...
  return (
    <div>
      ...
      <Modal
        title={
          editingAddress ? "Chỉnh sửa địa chỉ" : "Chọn địa chỉ trên bản đồ"
        }
        open={isModalVisible}
      ...
      >
        ...
        <Text type="secondary" style={{ display: "block", marginBottom: 16 }}>
          👉 Click vào đúng vị trí nhà của bạn trên bản đồ để tự động điền địa
          chỉ
        </Text>
